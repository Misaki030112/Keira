...
public class PlayerStatsTools {
    ...
    public String getPlayerInfo(@ToolParam(description = "Player name or UUID") String playerName) {
        AusukaAiMod.LOGGER.debug("{} [tool:get_player_info] player='{}'", RequestContext.midTag(), playerName);
    ...
    public String healPlayer(@ToolParam(description = "Player name or UUID") String playerName) {
        AusukaAiMod.LOGGER.debug("{} [tool:heal_player] player='{}'", RequestContext.midTag(), playerName);
        ServerPlayerEntity player = findPlayer(playerName);
        if (player == null) return "❌ Player not found: " + playerName;
        ...
        MainThread.runSync(server, () -> {
            try {
                ...
                AusukaAiMod.LOGGER.info("{} [tool:heal_player] done player='{}'", RequestContext.midTag(), player.getName().getString());
            } catch (Exception e) {
                result.set("❌ Heal failed: " + e.getMessage());
                AusukaAiMod.LOGGER.error("Heal player failed", e);
            }
        });
        return result.get();
    }
    ...
    public String listOnlinePlayers() {
        ...
        MainThread.runSync(server, () -> {
            ...
            AusukaAiMod.LOGGER.info("{} [tool:list_online_players] count={}", RequestContext.midTag(), players.size());
        });
        return result.get();
    }
