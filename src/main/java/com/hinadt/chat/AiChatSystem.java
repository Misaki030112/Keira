            ...
            dispatcher.register(CommandManager.literal("ai")
                    ...
                    .then(CommandManager.literal("auto-msg")
                        .then(CommandManager.literal("toggle")
                            .executes(context -> {
                                ...
                                if (!AdminTools.isPlayerAdmin(server, player)) {
                                    player.sendMessage(Text.of("§c[AI Misaki] 只有管理员才能控制自动消息系统"));
                                    return 0;
                                }
                                ...
                                player.sendMessage(Text.of("§b[AI Misaki] " + result));
                        ...
                        .then(CommandManager.literal("status")
                            .executes(context -> {
                                ...
                                player.sendMessage(Text.of("§b[AI Misaki] 自动消息系统状态: " + 
                                    (enabled ? "§a启用" : "§c禁用")));
                                player.sendMessage(Text.of("§b[AI Misaki] 当前在线玩家: " + playerCount));
                ...
                .then(CommandManager.literal("new")
                    .executes(context -> {
                        ...
                        player.sendMessage(Text.of("§b[AI Misaki] §a✨ 已开始新的对话会话！"));
                        player.sendMessage(Text.of("§b[AI Misaki] §f会话ID: " + newSessionId));
                        player.sendMessage(Text.of("§b[AI Misaki] §f现在我们可以进行全新的对话，我会记住这次对话的上下文。"));
    ...
    private static void sendAiWelcomeMessage(ServerPlayerEntity player) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                server.execute(() -> {
                    String fallbackWelcome = "🤖 你好 " + player.getName().getString() + "！我是AI助手Misaki，" +
                        "可以帮助你管理物品、智能传送、记忆重要位置、建筑指导等。" +
                        "直接告诉我你需要什么帮助，我会智能理解并为你服务！✨";
                    player.sendMessage(Text.of("§b[AI Misaki] §f" + fallbackWelcome));
                });
            }
        });
    }
