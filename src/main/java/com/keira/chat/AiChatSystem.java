...
public class AiChatSystem {
    ...
    public static void initialize() {
        ...
        KeiraAiMod.LOGGER.info("AI chat system initialized.");
    }
    ...
    private static void handleAiChatMessage(ServerPlayerEntity player, String message) {
        ...
        KeiraAiMod.LOGGER.debug("[mid={}] [chat] Received player message: player={}, msg='{}'", messageId, playerName, message);
        ...
        task.whenComplete((response, ex) -> {
            ...
            if (ex != null) {
                ...
                if (cause instanceof CancellationException) {
                    // Timeout path
                    KeiraAiMod.LOGGER.warn("[mid={}] [chat] AI task timed out and was cancelled: player={}", messageId, playerName);
                    AiServices.server().execute(() -> {
                        MutableText t = Text.translatable("keira.ai.timeout");
                        Messages.to(player, Text.of("§c").copy().append(t));
                    });
                } else {
                    // Other failure
                    KeiraAiMod.LOGGER.warn("[mid={}] [chat] Message processing failed: player={}, err={}", messageId, playerName, cause.toString());
                    AiServices.server().execute(() -> {
            ...
            String out = (response == null || response.isEmpty())
                    ? Text.translatable("keira.ai.no_response").getString()
                    : response;
            AiServices.server().execute(() ->
                    Messages.to(player, Text.translatable("keira.ai.reply", out))
            );
        });
    }
