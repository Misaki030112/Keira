...
import com.keira.persistence.mapper.PlayerConnectionMapper;
import com.keira.persistence.mapper.PlayerProfileMapper;
import com.keira.util.PlayerLocales;
...
public final class PlayerTelemetryRecorder {
    ...
    public static void recordJoin(ServerPlayerEntity player, ClientConnection connection, ConnectedClientData clientData) {
        try (var session = MyBatisSupport.getFactory().openSession(true)) {
            ...
            KeiraAiMod.LOGGER.debug("Recorded player join: uuid={}, name={}, ip={}, locale={}", uuid, name, ip, locale);
        } catch (Exception e) {
            KeiraAiMod.LOGGER.warn("Failed to record player join: " + e.getMessage(), e);
        }
    }
