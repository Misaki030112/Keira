...
public final class ChatCommands {
    ...
    private static boolean isAiAssistantName(String name) {
        ...
        String[] forbidden = {"keira","ai","scanner"};
        ...
        task.whenComplete((response, ex) -> {
            if (ex != null) {
                ...
                if (cause instanceof CancellationException) {
                    KeiraAiMod.LOGGER.warn("[mid={}] /ai say timed out and was cancelled", messageId);
                    AiServices.server().execute(() -> Messages.to(player, Text.translatable("aim.say.error")));
                } else {
                    KeiraAiMod.LOGGER.warn("[mid={}] /ai say failed: {}", messageId, cause.toString());
                    AiServices.server().execute(() -> Messages.to(player, Text.translatable("aim.say.error")));
                }
                return;
            }
            ...
            KeiraAiMod.LOGGER.debug("[mid={}] [/ai say] Sending AI reply to player={}, len={}, preview='{}'",
                    messageId, player.getName().getString(), out.length(), preview);
            AiServices.server().execute(() -> Messages.to(player, Text.translatable("keira.ai.reply", out)));
        });
        return 1;
    }
}
