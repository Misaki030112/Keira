...
public class WeatherTools {
    ...
    public String changeWeather(
        ...
        KeiraAiMod.LOGGER.debug("{} [tool:change_weather] args type='{}' duration='{}' world='{}' freezeCycle={}",
        ...
        KeiraAiMod.LOGGER.debug("{} [change_weather] normalized type='{}' seconds={} world='{}'",
                RequestContext.midTag(), kind, seconds, worldId);

        // weather not supported in nether/end
        if (!supportsWeather(target)) {
            KeiraAiMod.LOGGER.debug("{} [change_weather] dimension has no weather world='{}'", RequestContext.midTag(), worldId);
        ...
        MainThread.runSync(server, () -> {
            try {
                ...
                KeiraAiMod.LOGGER.debug("{} [change_weather] applied world='{}' weather='{}' sec={} freezeCycle={}",
                        RequestContext.midTag(), worldId, kind, seconds, freezeCycle);

            } catch (Exception e) {
                KeiraAiMod.LOGGER.error("{} [change_weather] ERR_APPLY {}", RequestContext.midTag(), e.toString(), e);
    ...
    public String setTime(
        ...
        KeiraAiMod.LOGGER.debug("{} [tool:set_time] args time='{}' world='{}' freezeCycle={}",
        ...
        KeiraAiMod.LOGGER.debug("{} [set_time] parsed current={} -> newTod={} world='{}'",
        ...
        MainThread.runSync(server, () -> {
            try {
                ...
                KeiraAiMod.LOGGER.debug("{} [set_time] applied world='{}' timeOfDay={} key='{}' freezeCycle={}",
                ...
                KeiraAiMod.LOGGER.error("{} [set_time] ERR_APPLY {}", RequestContext.midTag(), e.toString(), e);
    ...
    public String getWorldState(
        ...
        KeiraAiMod.LOGGER.debug("{} [tool:get_world_state] args world='{}'",
        ...
        MainThread.runSync(server, () -> {
            try {
                ...
                KeiraAiMod.LOGGER.debug("{} [tool:get_world_state] world='{}' raining={} thundering={} tod={} key='{}'",
                        RequestContext.midTag(), worldId, raining, thundering, timeOfDay, timeKey(timeOfDay));
            } catch (Exception e) {
                KeiraAiMod.LOGGER.error("{} [tool:get_world_state] error {}", RequestContext.midTag(), e.toString(), e);
    ...
    private void addWeatherTimesIfAvailable(com.google.gson.JsonObject res, ServerWorld world) {
        try {
            ...
            KeiraAiMod.LOGGER.debug("{} [world_state] weatherTimes clear={} rain={} thunder={}",
